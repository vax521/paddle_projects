...
def load_text8():
    with open("F:/python_projects/nlp/examples/data/text8.txt", "r") as f:
        corpus = f.read().strip("\n")
    f.close()
...
def build_dict(corpus):
    # 统计每个不同的词的频率，用字典表示
    print(Counter(corpus).most_common(3))
    word_freq_dict = dict(Counter(corpus))
    # 将这个词典中的词，按照出现次数排序，出现次数越高，排序越靠前
    # 一般来说，出现频率高的高频词往往是：I，the，you这种代词，而出现频率低的词，往往是一些名词，如：nlp
    word_freq_dict = sorted(word_freq_dict.items(), key = lambda x:x[1], reverse = True)
    print(word_freq_dict[:3])
    ...
    for word,freq in word_freq_dict:
        currid = len(word2id_dict)
        word2id_dict[word] = currid
        word2id_freq[currid] = freq
        id2word_dict[currid] = word
...
def convert_corpus_to_id_seq(corpus, word2id_dict):
    return [word2id_dict[word] for word in corpus]


id_corpus = convert_corpus_to_id_seq(corpus, word2id_dict)
print(id_corpus[:10])
...
def buiuld_train_data(id_corpus, word2id_dict, word2id_freq, max_window_size=3, negative_sample_num=3):
...
dataset = buiuld_train_data(corpus, word2id_dict, word2id_freq)
    ...
    for epoch in range(epoch_num):
        ...
        for center_word, target_word, label in dataset:
            ...
            if len(center_word_batch) == batch_size:
                yield np.array(center_word_batch).astype("int64"),np.array(target_word_batch).astype("int64"), np.array(label_batch).astype("float32"),np.array(eval_word_batch).astype("int64")
            center_word_batch = []
            target_word_batch = []
            label_batch = []
            eval_word_batch = []
